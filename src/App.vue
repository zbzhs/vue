<template>
  <div
    ...
    :class="{
      ...
      'site-shell--selection': isSelectionContext,
    }"
  >
    <header
      v-if="showSiteNav"
    ...
    >
      <div v-if="isSelectionContext" class="selection-classic-nav">
        <button
        ...
        <RouterLink class="nav-brand-wordmark" :to="{ name: 'selectionAll' }" aria-label="DERING selection home">
          <img src="/logo/logo.png" alt="DERING" />
        </RouterLink>

        <nav class="nav-category-row" aria-label="达人选货品类">
          <RouterLink
        ...
        <div class="nav-actions nav-actions--selection">
          <RouterLink class="selection-nav-return" :to="{ name: 'selection' }">
            推荐中心
          </RouterLink>
          <RouterLink
            v-if="route.name !== 'selectionPicks'"
            class="selection-nav-return"
          ...
          <button
            v-if="selectionUser"
            class="selection-nav-return selection-nav-logout"
            type="button"
            @click="handleSelectionLogout"
          >
            退出登录
          </button>
        </div>
      ...
      <template v-else>
        <RouterLink class="nav-brand-wordmark" :to="navBrandTo" aria-label="DERING home">
          <img src="/logo/logo.png" alt="DERING" />
        </RouterLink>
...
const { currentUser, selectionUser, displayName, logout, logoutSelection } = useAuth()
...
const showSiteNav = computed(() => !route.meta.hideSiteNav && route.name !== 'selectionPicks')
const showNavActions = computed(() => !route.meta.hideNavActions && !isSelectionContext.value)
...
const navBrandTo = computed(() => (isSelectionContext.value ? { name: 'selectionAll' } : { name: 'home' }))
...
const navItems = computed(() => {
  ...
  return baseNavItems.map((item) => ({
    ...item,
    to: { name: 'selectionAll', query: { type: item.to.query.type } },
  }))
})
...
async function openProductFilters() {
  if (isSelectionContext.value) {
    await router.push({ name: 'selectionAll' })
    return
  }
...
function handleSelectionLogout() {
  logoutSelection()
